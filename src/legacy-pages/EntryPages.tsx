...
export function BrandPage() {
  return <main className="brand-page"><MistScene /><div className="brand-copy"><span className="eyebrow">双人叙事 · 延迟交换 · 共同记忆</span><h1>交换人生</h1><p className="brand-statement"><span>我的话语，一半毫无意义；</span><span>但我说出来，<wbr />是为了让另一半能够抵达你。</span></p><Link to="/start" className="enter-link">推开门，慢慢说 <ArrowRight size={18} /></Link></div></main>
}

export function StartPage() {
  const { state } = useDemo()
  return <main className="centered-page start-page"><Link to="/" className="back-link"><ArrowLeft size={16} /> 回到门外</Link><header className="page-heading"><span className="eyebrow">今天想从哪里开始？</span><h1>把一段人生，交给熟悉的人</h1><p>{state.authenticated ? `欢迎回来，${state.users[state.activeUserId].name}。` : '不需要立刻说完。每一步都会为你保存。'}</p></header><div className="entry-choice-grid"><Link to={state.authenticated ? '/exchanges/new' : '/auth?next=/exchanges/new'} className="entry-choice"><span className="entry-icon ochre"><Plus /></span><div><span>发起一封信</span><h2>创建一次交换</h2><p>选择一件共同经历，或把自己的一段人生先告诉对方。</p></div><ArrowRight /></Link><Link to="/join" className="entry-choice"><span className="entry-icon lavender"><KeyRound /></span><div><span>我收到了邀请</span><h2>输入邀请码</h2><p>加入一位熟悉的人为你留下的双人空间。</p></div><ArrowRight /></Link></div>{state.authenticated && <Link className="subtle-link" to="/home">直接进入我的交换首页</Link>}</main>
}
...
export function ProfilePage() {
  ...
  const submit = () => {
    ...
    setSaved(true)
    window.setTimeout(() => navigate(params.get('next') || '/home'), 500)
  }
...
export function JoinPage() {
  ...
  const join = () => {
    ...
    if (!state.authenticated) navigate(`/auth?next=${encodeURIComponent('/exchanges/exchange-winter-001/context')}`)
    else navigate('/exchanges/exchange-winter-001/context')
  }
  return <main className="centered-page join-page"><Link to="/start" className="back-link"><ArrowLeft size={16} /> 返回</Link><div className="narrow-content"><span className="eyebrow">来自熟悉的人</span><h1>输入邀请码</h1><p>邀请码只用于加入指定的双人空间，不会把你带到公开社交场所。</p><PaperPanel className="invite-checker"><TextField label="8 位邀请码" value={code} onChange={e => { setCode(e.target.value.toUpperCase()); setStatus('idle') }} placeholder="试试 WINTER26" maxLength={8} /><PrimaryButton onClick={check} disabled={!code || status === 'checking'}>{status === 'checking' ? '正在确认…' : '确认邀请码'}</PrimaryButton>{status === 'valid' && <InvitationPreview onJoin={join} />}{status === 'invalid' && <StatusBanner tone="error">没有找到这个邀请码，请检查字母和数字。</StatusBanner>}{status === 'expired' && <StatusBanner tone="warning">这枚邀请码已经失效，请联系邀请人重新生成。</StatusBanner>}{status === 'joined' && <StatusBanner tone="info">你已经加入过这次交换，可以直接回到交换首页。</StatusBanner>}</PaperPanel><div className="invite-demo-codes"><span>演示状态：</span><button onClick={() => setCode('WINTER26')}>有效</button><button onClick={() => setCode('EXPIRED')}>已失效</button><button onClick={() => setCode('JOINED')}>已加入</button><button onClick={() => setCode('WRONG000')}>无效</button></div></div></main>
}

function InvitationPreview({ onJoin }: { onJoin: () => void }) {
  return <div className="invitation-preview"><div className="inviter"><span className="avatar-large">丹</span><div><small>丹青邀请你</small><strong>一起回看：那个冬至的晚饭</strong></div></div><p>“有些细节，我们也许记得不一样。想和你慢慢交换一次。”</p><div className="invitation-meta"><span>共同事件</span><span>双方独立表达</span><span>只对你们两人开放</span></div><PrimaryButton onClick={onJoin}>确认加入 <ArrowRight size={17} /></PrimaryButton></div>
}
