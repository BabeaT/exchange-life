...
export function OrganizePage() {
  ...
  const run = (fail = false) => {
    ...
    window.setTimeout(() => {
      ...
      else { setText(state.activeUserId === 'user-a' ? '我一直记得那个冬至。厨房的玻璃蒙着水汽，你在桌边一只一只地摆碗。那时我忙着收拾行李，以为自己没有回头。后来才明白，我记住的不是离开，而是门口那盏一直亮着的灯。具体是哪一年，我已经记不太清了。' : '那天，我很早就开始准备晚饭。你说晚上要赶车，我怕耽误你，所以一直没有问能不能多留一会儿。我记得你在门口系围巾时笑了一下。你走后，我把多盛的那碗汤放在窗边，等它慢慢凉下来。'); setPhase('result') }
    }, 950)
  }
...
export function ComposePage() {
  ...
  const [title, setTitle] = useState(state.activeUserId === 'user-a' ? '门口那盏灯' : '窗边那碗汤')
  return <div className="page compose-page"><header className="page-heading inline-heading"><div><span className="eyebrow">图文信件</span><h1>把确认过的文字，放进一封信里</h1><p>插图是叙事想象，不会被当作真实影像。</p></div><StatusBanner tone="success">文字已由本人确认</StatusBanner></header><div className="compose-layout"><aside className="compose-controls"><PaperPanel><label className="field"><span>信件标题</span><input value={title} onChange={e => setTitle(e.target.value)} /></label><span className="control-label">图文比例</span><TextImageRatioControl value={layout} onChange={setLayout} /></PaperPanel><PaperPanel><span className="control-label">模拟事件插图</span><div className="illustration-options">{[0, 1, 2].map(item => <button key={item} className={illustration === item ? 'selected' : ''} onClick={() => setIllustration(item)}><StoryIllustration variant={item} label={`插图方案${item + 1}`} /><span>方案 {item + 1}</span></button>)}</div><SecondaryButton onClick={() => setIllustration((illustration + 1) % 3)}><RotateCcw size={16} /> 更换模拟插图</SecondaryButton></PaperPanel></aside><div className="compose-preview"><LetterPreview letter={{ title, text: draft.organizedText, layout, illustration }} /></div></div><div className="sticky-actions"><SecondaryButton onClick={() => navigate(`/exchanges/${state.exchange.id}/organize`)}>修改文字</SecondaryButton><PrimaryButton onClick={() => { actions.toast('图文信件候选已保存'); navigate(`/exchanges/${state.exchange.id}/preview?title=${encodeURIComponent(title)}&layout=${layout}&art=${illustration}`) }}>进入完整预览 <ArrowRight size={17} /></PrimaryButton></div></div>
}
...
export function PreviewPage() {
  ...
  const draft = state.exchange.drafts[state.activeUserId]
  const letter = { title: params.get('title') || (state.activeUserId === 'user-a' ? '门口那盏灯' : '窗边那碗汤'), text: draft.organizedText, layout: (params.get('layout') || 'balanced') as 'text' | 'balanced' | 'image', illustration: Number(params.get('art') || 0) }
...
export function SendSettingsPage() {
  ...
  const send = () => {
    ...
    window.setTimeout(() => {
      ...
      actions.sendLetter({ title: state.activeUserId === 'user-a' ? '门口那盏灯' : '窗边那碗汤', text: draft.organizedText, layout: 'balanced', illustration: state.activeUserId === 'user-a' ? 0 : 1, sentAt: new Date().toISOString() })
