...
export function LetterPreview({ letter }: { letter: Pick<Letter, 'title' | 'text' | 'layout' | 'illustration'> }) {
  const paragraphs = letter.text.split('\n').filter(Boolean)
  return <article className={`letter-paper layout-${letter.layout}`}><header><span>交换人生 · 一封只给你的信</span><h1>{letter.title}</h1></header><StoryIllustration variant={letter.illustration} label="AI 生成的叙事插图" /><div className="letter-copy">{(paragraphs.length ? paragraphs : [letter.text]).map((text, index) => <p key={index}>{text}</p>)}</div><footer><span>叙事插图 · 非真实影像</span><span>这封信只包含寄信人确认公开的内容</span></footer></article>
}
...
export function LetterReader({ letter, sender }: { letter: Letter; sender: string }) {
  ...
  return <div className="letter-reader"><div className="reader-toolbar"><span>{sender} 写给你的信</span><div className="narration-control"><button onClick={() => setPlaying(value => !value)} aria-label={playing ? '暂停朗读' : '开始朗读'}>{playing ? <Pause size={16} /> : <Play size={16} />}</button><Volume2 size={15} /><span>{playing ? '温暖中性声 · 正在朗读' : '主动开启语音读信'}</span></div></div><div className="reader-page">{pages[page]}</div><div className="reader-nav"><SecondaryButton disabled={page === 0} onClick={() => setPage(page - 1)}><ChevronLeft size={16} /> 上一页</SecondaryButton><span>{page + 1} / {pages.length}</span><PrimaryButton disabled={page === pages.length - 1} onClick={() => setPage(page + 1)}>下一页 <ChevronRight size={16} /></PrimaryButton></div><div className="reading-progress"><span style={{ width: `${((page + 1) / pages.length) * 100}%` }} /></div></div>
}
