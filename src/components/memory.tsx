...
export function ViewpointComparison() {
  return <div className="mode-a-nodes">{modeAEventNodes.map((node, index) => <article className="mode-a-node" key={node.title}><header><span>节点 {index + 1}</span><h2>{node.title}</h2></header><div className="mode-a-perspectives"><section className="viewpoint a"><span>林夏的视角</span><p>{node.linxia}</p></section><section className="viewpoint b"><span>丹青的视角</span><p>{node.danqing}</p></section></div><dl className="node-reading"><div><dt>共同事实</dt><dd>{node.fact}</dd></div><div><dt>不同理解</dt><dd>{node.difference}</dd></div><div><dt>当时没有看见</dt><dd>{node.unseen}</dd></div></dl></article>)}</div>
}

export function MemoryFragment() {
  const { state } = useDemo()
  return <article className="memory-fragment"><StoryIllustration variant={2} label="列车启动后，林夏隔着车窗看见丹青仍站在站台上" /><div><span className="eyebrow">共同记忆碎片 · 001</span><h2>两个没有说出口的害怕</h2><p><strong>共同事实：</strong>两个人都支持这次离开，也都担心离开会改变彼此的关系。</p><p><strong>仍然存在的不同：</strong>林夏需要的是信任与独立空间；丹青仍然会担心，只是在学习改变表达关心的方式。</p><blockquote>“沉默并不只属于自己。”</blockquote><div className="fragment-meta"><span>关联事件：我离开家的那一年</span><span>故事时间：2024年8月</span><span>共同物件：装着家中食物和生活用品的行李箱</span><span>{state.users['user-b'].name} × {state.users['user-a'].name}</span></div></div></article>
}
